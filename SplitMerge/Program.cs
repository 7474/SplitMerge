...
namespace SplitAndMerge
{
    ...
    class StrtodFunction : ParserFunction
    {
        ...
        {
            double num;
            // XXX 関数を登録していくような作りにする
            if (DiceRoll.TryRoll(Item, out num))
            {
                return num;
            }
    ...
    class DiceRoll
    {
        ...
        public static bool TryRoll(string item, out double num)
        {
            num = Double.NaN;
            var diceReg = new Regex("^\\s*([1-9][0-9]{0,2})d([1-9][0-9]{0,8})\\s*$", RegexOptions.IgnoreCase);
            var diceTest = diceReg.Match(item);
